...
class ux_tx_tfclifegroups_pi1 extends tx_tfclifegroups_pi1 {
	...
	function main($content,$conf)	{
		...
		$this->perPage = is_numeric($this->conf['perPage']) ? $this->conf['perPage'] : $this->perPage;
		$this->columns = is_numeric($this->conf['columns']) ? $this->conf['columns'] : $this->columns;
		$this->perCol = $this->perPage / $this->columns;
		...
		switch($this->lConf['view_mode']){
			case 1:
				if(! (isset($this->piVars['showUid']) == 0)) {
					return $this->pi_wrapInBaseClass($this->detailView($content,$conf));
				} elseif(!isset($this->piVars['selCategory'])) {
	...
	function listView($content,$conf)	{
		...
		if ($this->piVars['new'] == 1) {
			$fullTable.=$this->pi_list_makelist($res, ' class="xds_smallgroup_base_table fellowship" cellpadding="0" cellspacing="0"');
		} else {
		...
		if ($this->piVars['new'] == 1) {
			$this->strHead = $arBrowse[0].$arBrowse[1].'</span></span></div>';
		} else {
		...
		if ($this->piVars['new'] == 1) {
			$fullTable = str_replace('<div class="tx-tfclifegroups-pi1-listrow">', '<div class="tx-tfclifegroups-pi1-listrow fellowship"><span>'
				. $this->internal["res_count"] . ' '
				. $this->pi_getLL('pi_list_number_group','Groups found')
				. '</span>', $fullTable);
			$fullTable = $fullTable . $this->strHead;
		} else {
	 ...
	 *
	 */
	function detailView($content, $conf) {
		if($this->templateFileContent){
			$this->arRecurrences = $this->setRecurrences();
			$this->arDays = $this->setDays();
			$this->arInterests = $this->setInterests();
			...
			$res = $this->pi_getRecord('tx_tfclifegroups_lifegroups', $this->piVars['showUid']);
			$this->internal['currentRow'] = $res;

			$dataRowContent = $this->cObj->getSubpart($this->templateFileContent,'###DETAIL###');

			$subStrArray = Array();
			$subStrArray['###NAME###'] = $this->getFieldContent("title");
			$subStrArray['###LEADER_PHONE###'] = $this->getFieldContent("leader_phone");
			$subStrArray['###LEADER_EMAIL###'] = '<a href="mailto:' . $this->getFieldContent("leader_email") . '">' . $this->getFieldContent("leader_email") . '</a>';
	...
	function pi_list_row($c)	{
		...
		if($this->templateFileContent){
			...
			$subStrArray = Array();
			if ($this->piVars['new'] == 1) {
				$subStrArray['###NAME###'] = $this->pi_list_linkSingle($this->getFieldContent("title"),$this->internal['currentRow']['uid'],1,array('new'=>1));
			} else {
				$subStrArray['###NAME###'] = $this->getFieldContent("title");
			}
			$subStrArray['###LEADER_PHONE###'] = $this->getFieldContent("leader_phone");
			...
			if ($this->piVars['new'] == 1) {
				$point = '';
				if(strlen($this->getFieldContent("descr")) >= 50) {
					$point = $this->pi_getLL('pi_list_description_end_point','...');
				}
				$subStrArray['###DESC###'] = substr($this->getFieldContent("descr"), 0, 50) . $point;
			} else {
				$subStrArray['###DESC###'] = $this->getFieldContent("descr");
			}
	...
	function getDays($conf) {
		...
		$html = '<select class="input_select" name="tx_tfclifegroups_pi1[selDay]" style="width: 200px">
	...
	function getInterests($conf) {
		...
		$html = '<select class="input_select" name="tx_tfclifegroups_pi1[selInterest]" style="width: 200px">
	...
	function getAges($conf) {
		...
		$html = '<select class="input_select" name="tx_tfclifegroups_pi1[selAge]" style="width: 200px">
